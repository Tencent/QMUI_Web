...
module.exports = function(gulp, common) {
  gulp.task('watch', watchTaskDesciption, function () {
    ...
    common.plugins.util.log(common.plugins.util.colors.green('QMUI Watch: ') + 'QMUI 进入自动监听');
    ...
    _compassWatch.on('change', function() {
      ...
      common.plugins.util.log(common.plugins.util.colors.green('QMUI Compass: ') + '进行 Compass 编译');
    });
      ...
      gulp.src(_dir)
          .pipe(common.plugins.plumber({
            errorHandler: function(_error) {
              common.plugins.util.log(common.plugins.util.colors.red('QMUI Min Image: ') + _error);
      ...
      common.plugins.fileSync(_independentImagesSourcePath, _independentImagesResultPath, {
        ...
        beforeAddFileCallback: function(_fullPathSrc) {
         ...
         if(!common.lib.isElementInArray(justBeforeAddedImage, _absoluteMinImageFilePathMd5)) {
           ...
           common.plugins.util.log(common.plugins.util.colors.green('QMUI Min Image: ') + '对 ' + _absoluteMinImageFilePath + ' 进行图片压缩');
        ...
        addFileCallback: function(_fullPathSrc, _fullPathDist) {
         ...
         if(!common.lib.isElementInArray(justAddedImage, _absoluteMinImageFilePathMd5)) {
           ...
           _outputEmptyForSyncImageIfNeed();
           common.plugins.util.log(common.plugins.util.colors.green('QMUI Sync Image: ') + '同步增加文件到 ' + _absoluteMinImageFilePath);
         }
        },
        deleteFileCallback: function(_fullPathSrc, _fullPathDist) {
         _outputEmptyForSyncImageIfNeed();
         common.plugins.util.log(common.plugins.util.colors.green('QMUI Sync Image: ') + '同步删除文件 ' + path.resolve(_fullPathDist));
        },
        updateFileCallback: function(_fullPathSrc, _fullPathDist) {
         ...
         if(!common.lib.isElementInArray(justAddedImage, _absoluteMinImageFilePathMd5)) {
           justAddedImage.push(_absoluteMinImageFilePathMd5);
           _outputEmptyForSyncImageIfNeed();
           common.plugins.util.log(common.plugins.util.colors.green('QMUI Sync Image: ') + '同步更新文件到 ' + _absoluteMinImageFilePath);
         } else {
    ...
    _minImageWatcher.on('change', function(event) {
      ...
      if(event.type !== 'deleted' && !common.lib.isElementInArray(justAddedImage, _minImageFilePathMd5)) {
        ...
        common.plugins.util.log(common.plugins.util.colors.green('QMUI Min Image: ') + '对 ' + _minImageFile + ' 进行图片压缩');
    ...
    if(common.config.openIncludeFunction) {
      ...
      _includeWatcher.on('change', function(event) {
        common.plugins.util.log('');
        common.plugins.util.log(common.plugins.util.colors.green('QMUI Include: ') + '模板 ' + event.path + ' was ' + event.type);
      });
    }
